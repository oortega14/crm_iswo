...
type AuthSlice = { tenant?: Tenant | null }

/** Bloquea rutas comerciales cuando la sesión es del tenant plataforma. */
export function enforcePlatformRouteAccess(auth: AuthSlice, pathname: string): void {
...
export function requirePlatformTenant(auth: AuthSlice): void {
...
export function requireCommercialTenant(auth: AuthSlice): void {
  if (isPlatformTenant(auth.tenant)) {
    throw redirect({ to: PLATFORM_HOME })
  }
}
